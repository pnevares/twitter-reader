...
function TweetServer(config) {
    ...
    redisClient.on('error', function(error) {
        console.log('(TweetServer) redis error: ' + error);
    });
    ...
    this.server = http.createServer(function(req, res) {
        ...
        switch(parsed.pathname) {
            case '/tweet':
                ...
                break;
            default:
                console.log('(TweetServer) bad endpoint');
        ...
        function loadedFromCache(error, response) {
            if(error) {
                ...
                console.log('(TweetServer) cache hit for tweet:' + tweetId);
                ...
                console.log('(TweetServer) cache miss for tweet:' + tweetId);
        ...
        function validatedUsingSearch(error, response) {
            if(error || response.posts.length === 0) {
                if(error) {
                    console.log('(TweetServer) winchatty module error: ' + error);
                }
        ...
        function loadedFromTwitter(error, response) {
            ...
            if(error || !('html' in response)) {
                console.log('(TweetServer) twitter module error:' + error);
                outputResponse(res, 500, '500 Internal Server Error');
            } else {
                var tweetBody = response.html;
                outputResponse(res, 200, tweetBody);

                console.log('(TweetServer) caching tweet:' + tweetId);
