    ...
    this.server = http.createServer(function(req, res) {
        ...
        switch(parsed.pathname) {
            case '/tweet':
                ...
                break;
            case '/ping':
                outputResponse(res, 200, 'pong! v' + version);
                break;
            case '/latest':
                redisClient.get('most-recent-tweet', readLatestTweet);
                break;
            default:
        ...
        function loadedFromTwitter(error, response) {
            ...
            if(error || !('html' in response)) {
                ...
                redisClient.zincrby('daily-served', 1, dateStamp);
                redisClient.zincrby('tweet-requests', 1, tweetKey);
                redisClient.set('most-recent-tweet', tweetKey);
            }
        }
...
TweetServer.prototype.start = function() {
    ...
    this.server.listen(port, function() {
        console.log(moment().utc().format() + ' TweetServer v' + version + ' is listening on port ' + port);
    });
};
